...
6. `_workspace/00_index.md`를 생성한다. 형식:
   ```markdown
   # Repo Merger — 작업 인덱스
   - **작업일**: {yyyy-mm-dd}
   - **통합 목적**: {purpose}
...
Analyzer 완료 후 `_workspace/01_analyzer_report.md`가 생성되었는지 확인하고, `00_index.md`의 산출물 로그에 행을 추가한다:
...
모든 Step 완료 후 `00_index.md`의 산출물 로그에 행을 추가한다:
...
`_workspace/`의 모든 산출물(`01_analyzer_report.md` ~ `04_verifier_final_report.md`)을 읽고, `_workspace/05_integration_summary.md`를 생성한다. 형식:
...
1. 최종 리포트와 통합 요약서를 사용자에게 제시한다.
2. 새 프로젝트 디렉토리에서 `git init`을 실행할지 사용자에게 묻는다.
3. `_workspace/`는 보존한다 (감사 추적용). 이후 이 프로젝트를 열었을 때 `_workspace/00_index.md`와 `05_integration_summary.md`로 맥락을 파악할 수 있음을 안내한다.
