...
3. **세션 메모리 로드:**
   - `idea-forge/_memory/` 디렉토리가 존재하는지 확인한다.
   - 존재하면 `idea-forge/_memory/session-log.md`를 Read로 읽어 이전 세션의 학습을 파악한다.
...
4. **세션 슬러그 결정:**
   - 사용자 요청에서 핵심 키워드를 추출하여 slug를 생성한다 (예: "반려동물 간식" → `pet-snack`)
   - 같은 날 같은 slug가 이미 `idea-forge/sessions/`에 있으면 `-2` 접미사를 붙인다.
...
3. **최종 보고서 통합:** 4개 산출물을 `idea-forge/sessions/{날짜}-{slug}/final-report.md`로 통합한다:
...
4. **세션 메모리 저장:**
   ...
   `idea-forge/_memory/session-log.md`에 append한다 (기존 내용 보존 + 새 세션 추가):
...
`_workspace/` 전체를 삭제한다. 영구 산출물은 이미 `idea-forge/`에 저장되었으므로 안전하다.

1. `_workspace/` 디렉토리 전체 삭제:
   ```
   rm -rf _workspace/
   ```
...
검증 항목:
1. `idea-forge/sessions/{날짜}-{slug}/final-report.md`에 필수 섹션이 모두 있는가
2. `_workspace/` 디렉토리가 삭제되었는가
3. `idea-forge/_memory/session-log.md`가 올바른 형식인가
4. git log에 체크포인트 커밋이 올바른 순서로 존재하는가

검증 통과 시 사용자에게 결과 보고 + 승격 제안. 실패 시 문제 항목과 수정 방법을 제시한 후 재검증을 제안한다.
...
최종 보고서: idea-forge/sessions/{날짜}-{slug}/final-report.md
세션 메모리 저장 완료. 다음 실행 시 자동 반영.

### 아이디어 승격
승격할 아이디어 번호를 입력하세요 (예: "1, 3" 또는 "전부").
승격하면 `idea-forge/ideas/{slug}/card.md`로 독립 관리됩니다.
나중에 `/promote-idea`로도 승격할 수 있습니다.
```

**후속 액션:**
- 승격 요청 시 → `/promote-idea` 스킬 실행
- "심화해줘" → 승격된 아이디어의 `deep-dive.md` 생성
- "다른 카테고리로 재조사하시겠습니까?"
...
[Phase 1: 준비 + 메모리 로드]
   │
   ├── _workspace/ 생성 (임시)
   ├── idea-forge/_memory/session-log.md → 이전 학습 추출
   ↓
[Phase 2: 팀 구성]
   │
   ├── TeamCreate(trend-scout, market-analyst, idea-architect)
   ↓
[Phase 3~5: _workspace/에서 작업] → Phase별 git commit
   │
   ├── _workspace/01_*.md, 02_*.md, 03_*.md
   ↓
[Phase 6: 통합 + 메모리]
   │
   ├── _workspace/ 산출물 → idea-forge/sessions/{날짜}-{slug}/final-report.md
   ├── idea-forge/_memory/session-log.md append
   ↓
[Phase 7: Cleanup]
   │
   ├── rm -rf _workspace/
...
[Phase 8: 검증 + 승격 제안]
   │
   ├── /verify-output → PASS/FAIL
   ├── "승격할 아이디어 번호?" → /promote-idea
   └── idea-forge/ideas/{slug}/card.md 생성
```
...
2. Phase 1: slug=`side-project`, 메모리 없음
3. Phase 3~5: `_workspace/`에서 작업 + Phase별 commit
4. Phase 6: `idea-forge/sessions/2026-04-05-side-project/final-report.md` + 메모리 저장
5. Phase 7: `rm -rf _workspace/` + commit
6. Phase 8: 검증 PASS → "승격할 아이디어 번호?" → 사용자가 "1" 입력
7. `/promote-idea` → `idea-forge/ideas/vert-agent/card.md` 생성 + `_index.md` 업데이트

### 정상 흐름 — 두 번째 세션 + 아이디어 성장
1. 사용자: "지난번이랑 다른 방향으로 아이디어 더 만들어줘"
2. Phase 1: `idea-forge/_memory/` 로드, slug=`different-direction`
3. Phase 3~5: 이전과 다른 방향 + 발산적 재검토
4. Phase 6: `idea-forge/sessions/2026-04-08-different-direction/final-report.md`
5. Phase 7~8: cleanup + 검증 + 승격
6. 이후: "vert-agent 심화해줘" → `idea-forge/ideas/vert-agent/deep-dive.md` 생성

### 에러 흐름
1. Phase 3에서 trend-scout 실패, market-analyst만으로 진행
2. Phase 6: final-report에 "트렌드 리서치 미완료" 명시
3. Phase 7: `_workspace/` 삭제 정상 진행
4. Phase 8: 검증 "트렌드 요약" 불완전 → FAIL 보고 + 원인 안내
